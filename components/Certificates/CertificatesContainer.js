...
export default function CertificatesContainer({certificate, harvard}) {
    return <div className='py-4 md:py-5 lg:py-6 mb-2 md:mb-3 lg:mb-4'>
        <div className="flex flex-col md:flex-row items-center justify-around text-white">
            <div className="md:w-[40%] mx-auto px-2">
                ...
                <a href={certificate.link} target='_blank' title='View Certificate in New Tab'>
                    <button className='flex items-center justify-center rounded-md text-base md:text-lg lg:text-xl gap-2 transition ease-linear hover:text-blue-700 hover:bg-white border border-white w-full py-2 lg:py-3 my-2'><ExternalLinkIcon className='w-4 md:w-5 lg:w-6' /> <span>View Certificate</span></button>
                </a>
            </div>

            <div className='md:w-[50%] mx-auto px-4 text-base md:text-lg lg:text-xl roboto'>
        ...
        <details className='details-bg rounded-md text-base md:text-lg lg:text-xl'>
            <summary className='roboto details-bg cursor-pointer p-4 mt-2 rounded-md text-white'>{certificate.name} - Course Overview</summary>
